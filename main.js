...
async function addDependencies({dependencies = DEPENDENCIES, devDepenencies = DEV_DEPENDENCIES, cwd = APP_PATH} = {}) {
  const cmd = 'yarn'
  const args = [
    'add',
    ...dependencies
  ...
  return await spawn(cmd, devArgs, { cwd })
}
...
function deleteCraFiles(appPath) {
  const cmd = 'rm';
  const filesToDelete = [
'logo.svg','App.js','App.test.js','App.css','index.js','index.css'
  ]
